...
void loop() {
  ...
  if (isDoorClosed != previousDoorState) {
    ...
    // Send the updated door state as a heartbeat
    send_heartbeat(isDoorClosed ? "closed" : "open");
...
void send_heartbeat(String doorStatus) {
  if (client.connected()) {
    // Prepare the heartbeat message with door status
    String heartbeatMsg = "ESP32 is alive. Door is " + doorStatus;
    
    client.publish("smartLock/heartbeat", heartbeatMsg.c_str());
...
void reconnect_mqtt() {
  ...
  if (now - lastReconnectAttempt > 5000) { // 5 seconds
    lastReconnectAttempt = now;

    Serial.print("Attempting MQTT connection...");
    if (client.connect(clientId.c_str())) { // Use the unique client ID
